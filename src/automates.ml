...
type pre_automate_det = {
  (* used only during its building, the actual one is easier to represent in a file *)
  ...
  let ajouter_fin (elem : int) : unit =
    ...
    | [ e1; e2 ] ->
        ...
        if e1 = safe_token then a_det.fin.(elem) <- Some e2
          ...
          print_char '"';
          print_newline ();
          failwith "A syntax can't have more than one output"
          (* il a plus d'un élément final *))
