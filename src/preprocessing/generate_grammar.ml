...
let alphanumerical_maj (c : char) : bool =
  65 <= Char.code c && Char.code c <= 90
(** teste si le caractère [c] est une minuscule *)

(** teste si le caractère [c] est une minuscule *)
let alphanumerical_min (c : char) : bool =
...
(** renvoie le nom de toutes les règles et le reste des caractères dans la ligne
    [s] *)
let get_rule_name (s : char list) : string * char list =
  let rec get_rule_name_aux (s : char list) (revname : char list) :
      string * char list =
    match s with
...
let get_derivations (s : char list) : pattern list =
  ...
  *)
  let split_derivation (s : char list) : pattern =
    (** sépare les différents non terminaux de [s] sur le motif '\[a-z\]
        \[A-Z\]' et les ajoute à [rev_rule_name] *)
    let rec split_derivation_aux (s : char list) (rev_rule_name : char list) :
        pattern =
      match s with
      | [] ->
  ...
  (** sépare les disjonctions de non terminaux de [s] sur le motif ' | ' et les
      ajoute à [rev_derivation] *)
  let rec split_derivations (s : char list) (rev_derivation : char list) :
      pattern list =
    match s with
...
let rule_filter (l : string) : bool =
  if String.length (String.trim l) < 2 then false
  else l.[0] <> '/' && l.[1] <> '/'
...
let is_terminal (r : rule) : bool =
  match r with _, [ [ s ] ] -> not (alphanumerical_maj s.[0]) | _ -> false
...
(** récupère les non terminaux de la grammaire [g] *)
let non_terminals (g : grammar) : rule list =
  List.filter (fun (r : rule) -> not (is_terminal r)) g
