...
let get_safe_symbol (term : rule list) : string =
  fst
    (List.nth
       (List.filter (fun (r : rule) -> (string_of_terminal r).[0] = 's') term)
       0)

let get_unparsed_symbol (term : rule list) : string list =
  List.map fst
    (List.filter (fun (r : rule) -> (string_of_terminal r).[0] = '_') term)
  ...
  List.iter
    (fun (s, p) ->
      output_string file
        ("(\""
        ^ String.escaped
            (let s = printable_string_of_terminal (s, p) in
             if (string_of_terminal (s, p)).[0] = '\'' then
...
let generate_file_symbols (g : grammar) (f_name : string) : unit =
  ...
  List.iter
    (fun (s, p) ->
      output_string output_file
        ("  | " ^ s ^ " -> \""
        ^ String.escaped (printable_string_of_terminal (s, p))
        ^ "\"\n"))
    t;
