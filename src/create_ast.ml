...
let rec merge_syntax (l : token list) : token list =
  ...
  | Syntax End :: Space :: Syntax Function :: q ->
      Syntax End_function :: merge_syntax q
  | Syntax End :: Space :: Syntax Interface :: q ->
      Syntax End_interface :: merge_syntax q
  | Syntax End :: Space :: Syntax Module :: q ->
      Syntax End_module :: merge_syntax q
  | Syntax End :: Space :: Syntax Program :: q ->
      Syntax End_program :: merge_syntax q
  | Syntax End :: Space :: Syntax Select :: q ->
      Syntax End_select :: merge_syntax q
  | Syntax End :: Space :: Syntax Subroutine :: q ->
      Syntax End_subroutine :: merge_syntax q
  | Syntax End :: Space :: Syntax Type :: q -> Syntax End_type :: merge_syntax q
  | Syntax End :: Space :: Syntax Where :: q ->
      Syntax End_where :: merge_syntax q
...
let rec is_in_list (l : token list) (t : token) : bool =
  match l with [] -> false | x :: q -> if x == t then true else is_in_list q t

(* prend tous les tokens jusqu'a untoken  *)
let get_up_to (l : token list) (stop_token : token list) :
    token list * token list =
  let rec aux (l1 : token list) (l2 : token list) : token list * token list =
    match l1 with
    | [] -> (List.rev l2, l1)
    | x :: q ->
        if is_in_list stop_token x then (List.rev l2, l1) else aux q (x :: l2)
  in
  aux l []
...
let rec compact_ast_list (l : ast list) : ast list =
  let rec compact_on_token (l1 : ast list) (t : token) : ast list =
    match l1 with
    | [] -> []
    | Noeud (x1, x2, x3) :: Noeud (y1, [], []) :: Noeud (z1, z2, z3) :: q
      when y1 = t ->
        compact_on_token
          (Noeud
             ( y1,
               [
                 Noeud (x1, compact_on_token x2 t, compact_on_token x3 t);
                 Noeud (z1, compact_on_token z2 t, compact_on_token z3 t);
               ],
               [] )
          :: q)
          t
    | Noeud (x1, x2, x3) :: q ->
        Noeud (x1, compact_on_token x2 t, compact_on_token x3 t)
        :: compact_on_token q t
  in
  List.fold_left compact_on_token l
    [
      Operateur Puissance;
      Operateur Fois;
      Operateur Division;
...
let rec create_ast (l : token list) : ast list * token list =
  ...
  | Identificateur x :: q1 ->
      let a2, q2 = create_ast q1 in
      (Noeud (Identificateur x, [], []) :: a2, q2)
  | Operateur x :: q1 ->
      let a2, q2 = create_ast q1 in
      (Noeud (Operateur x, [], []) :: a2, q2)
  | OperateurLogique x :: q1 ->
      let a2, q2 = create_ast q1 in
      (Noeud (OperateurLogique x, [], []) :: a2, q2)
  | Comparateur x :: q1 ->
      let a2, q2 = create_ast q1 in
      (Noeud (Comparateur x, [], []) :: a2, q2)
  ...
  | Syntax If :: q1 -> (
      ...
      | Syntax Else :: q4 -> (
          ...
          match q5 with
          | Syntax End_if :: q6 ->
              print_endline "else end if";
              let a7, q7 = create_ast q6 in
              ( Noeud (Syntax If, a2, a3) :: Noeud (Syntax Else, [], a5) :: a7,
                q7 )
  ...
  | Syntax Do :: Space :: Syntax While :: q1 ->
      let a2, q2 = create_ast q1 in
      let a3, q3 =
        (fun a ->
          match a with
  ...
  | Syntax End_do :: q1 -> ([], q1)
  | Parentheseouvrante :: q1 ->
      let a2, q2 = create_ast q1 in
      let a3, q3 = create_ast q2 in
      (Noeud (Parentheseouvrante, [], a2) :: a3, q3)
  | Parenthesefermante :: q1 -> ([], q1)
  | Syntax Print :: q1 ->
      let l2, q2 = get_up_to q1 [ NewLine; PointVirgule ] in
      let a3, q3 = create_ast l2 in
      let a4, q4 = create_ast q2 in
...
let rec set_variable_list_to_env (var_list : ast list) (env : environnement)
    (t : token) : environnement =
  match var_list with
  | [] -> env
  | Noeud (Identificateur nom, [], []) :: q ->
      let env2 = set_type env nom t in
...
let env_of_ast (ast1 : ast list) : environnement =
  let rec env_of_ast_inner (ast2 : ast list) (env : environnement) :
      environnement =
    match ast2 with
    | [] -> env
    | Noeud (Syntax x, [], l2) :: q
      when x = Real || x = Integer || x = Logical || x = Character ->
        let env2 = set_variable_list_to_env l2 env x in
        env_of_ast_inner q env2
    | Noeud (_, l1, l2) :: q ->
        let env2 = env_of_ast_inner l1 env in
        let env3 = env_of_ast_inner l2 env2 in
        env_of_ast_inner q env3
  in
  env_of_ast_inner ast1 []

let biblio_of_ast (ast1 : ast list) : Bibliotheques.libs =
  let rec biblio_of_ast_inner (ast2 : ast list) (biblio : Bibliotheques.libs) :
      Bibliotheques.libs =
    match ast2 with
    | [] -> biblio
    | Noeud (Syntax Print, _, _) :: q ->
        let biblio2 = Bibliotheques.add_lib biblio "<stdio.h>" in
        biblio_of_ast_inner q biblio2
    | Noeud (_, l1, l2) :: q ->
        let biblio2 = biblio_of_ast_inner l1 biblio in
        let biblio3 = biblio_of_ast_inner l2 biblio2 in
        biblio_of_ast_inner q biblio3
  in
  biblio_of_ast_inner ast1 []
