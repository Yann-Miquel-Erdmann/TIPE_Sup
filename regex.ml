...
| C of regex*int*token*int*bool (* C for complex, searches the regular expression ex : (1|2|3|4|5|6|7|8|9|0) *)
;;
...
let rec match_regex (a:automaton) (s:search) (c: char) : automaton =
  match a, s with
  | N _, (_, index, _)-> a
  | C (reg, -2, t, index, b), (_, indexfin, _) ->
    begin
      match index_list reg index with
      | Litteral (l, i) ->
        begin
          if index_list l i == c then
            if (i+1) == List.length l then
              C (reg, -2, t, index+1, b)
            else
              C (replace_index reg index (Litteral (l, (i+1))), -2, t, index, b)
          else
            C (reg, indexfin, t, index, false)
        end
      | Range l ->
        begin
          let rec checkrange (l:(int*int) list) : bool =
            ...
            | [] -> false
          in if checkrange l then C(reg, -2, t, index+1, b) else C(reg, indexfin, t, index+1, false)
        end
      | AllChars -> C(reg, -2, t, index+1, b)
      ...
      | UnPlus (r, count) -> begin 
        ...
        let result =
          if index+1 == List.length reg || count == 0 then
            ...
            (* passer au suivant *)
            let a2 = match_regex (C([index_list reg (index+1)], -2, t, index, b)) s c in
            match a2 with
            | N _ -> failwith "impossible"
            | C (_, -2, _, _ , false) | C (_, _, _, _, true) -> true
            | C (_, i, _, _, false) -> false
        in
        if not result then
          let a2 = match_regex (C([r], -2, t, 0, b)) s c in
          match a2 with
          | N _ -> failwith "impossible"
          | C(r2, -2, t2, i, b2) ->
            if i > 0 then
              (* recommencer au début *)
              C(replace_index reg index (UnPlus ((index_list (reset_regex r2) 0), (count+1))), -2, t, index, b)
            else
              (* continuer sur le même *)
              C(replace_index reg index (UnPlus ((index_list r2 0), count)), -2, t, index, b)
          | C(_, failindex, _, i, _) ->
            if count == 0 then 
              (* fail *)
              C(reg, indexfin, t, index, false)
          ...
          C(replace_index reg index (UnPlus (r, count)), -2, t, index+1, b)
       end 
      | ZeroPlus (r, count) -> 
        let result =
          (* tester suivant *)
...
gen_regex "\".*\"";;
gen_regex "'.*'";;
let num_reg = gen_regex "[0-9]+";;

match_regex (C(num_reg, -2, Space, 0, false)) (0, 0, false) 'a'